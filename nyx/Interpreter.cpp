...
nyx::Value calcUnaryExpr(nyx::Value &lhs, Token opt) {
    ...
    if (opt == TK_MINUS) {
        ...
        case nyx::NyxDouble:
            return nyx::Value(nyx::NyxDouble, -std::any_cast<double>(lhs.data));
        default:
...
nyx::Value calcBinaryExpr(nyx::Value &lhs, Token opt , Value& rhs) {
    ...
    case TK_PLUS:
        if (lhs.type == nyx::NyxInt && rhs.type == nyx::NyxInt) {
            ...
            result.data =
                std::any_cast<double>(lhs.data) + std::any_cast<double>(rhs.data);
        } else if (lhs.type == nyx::NyxInt &&
            rhs.type == nyx::NyxDouble) {
            result.type = nyx::NyxDouble;
            result.data =
                std::any_cast<int>(lhs.data) + std::any_cast<double>(rhs.data);
        } else if (lhs.type == nyx::NyxDouble &&
            rhs.type == nyx::NyxInt) {
            result.type = nyx::NyxDouble;
            result.data =
                std::any_cast<double>(lhs.data) + std::any_cast<int>(rhs.data);
        }
        break;
    case TK_MINUS:
        if (lhs.type == nyx::NyxInt && rhs.type == nyx::NyxInt) {
    ...
    case TK_DIV:
        if (lhs.type == nyx::NyxInt && rhs.type == nyx::NyxInt) {
            result.type = nyx::NyxDouble;
            result.data =
                (double)std::any_cast<int>(lhs.data) / std::any_cast<int>(rhs.data);
        } else if (lhs.type == nyx::NyxDouble &&
            rhs.type == nyx::NyxDouble) {
            result.type = nyx::NyxDouble;
            result.data =
...
nyx::Value BinaryExpr::eval(nyx::GlobalContext* gctx, nyx::LocalContext* lctx) {
    nyx::Value lhs = this->lhs ? this->lhs->eval(gctx, lctx) : nyx::Value(nyx::NyxNull);
    nyx::Value rhs = this->rhs ? this->rhs->eval(gctx, lctx) : nyx::Value(nyx::NyxNull);
    Token opt = this->opt;
    if (!lhs.isNyxNull()) {
        // Unary evaluating
        if (rhs.isNyxNull()) {
...
nyx::Value FunCallExpr::eval(nyx::GlobalContext* gctx, nyx::LocalContext* lctx) {
    nyx::Value result;
